...
AuthorizerRedis.prototype._checkPassAndGetACL = function(client, user, pass, cb) {
  var userArr = user.split('_');
  if (userArr.length !== 2) {
      client.logger.warn('parse user info ' + user);
      cb(null, false);
      return;
  }
  var appId = userArr[0];
  var authTable = 'user:auth:' + appId;
  var authkey = user;
  var aclTable = 'user:acl:' + appId;
  var aclkey = user;
  //get from redis
  this.redisClient = this._buildClient();
  var that = this;
  this.redisClient.pipeline([
    ['hget', authTable, authkey],
    ['hget', aclTable, aclkey]
