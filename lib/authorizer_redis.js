...
AuthorizerRedis.prototype._authenticate = function(client, user, pass, cb) {
  ...
  this._checkPassAndGetACL(client, user, pass, userInfo, function(err, result) {
    cb(err, result);
  });
};
  ...
  this.redisClient.pipline([
    ...
    if (!result[0][0]) {
      ...
      try {
        ...
        client.logger.warn('parse user auth info ' + result[0][1] + ' to json error');
        cb(null, false);
        return;
      }
      ...
      hasher({
        ...
        if (success) {
          ...
          try {
            ...
            client.logger.warn('parse user acl info ' + result[1][1] + ' to json error');
            aclInfo = {};
          }
